	...
	virtual void UpdateWidgetSize(int widget, Dimension *size, const Dimension &padding, Dimension *fill, Dimension *resize)
	{
		...
		switch (widget) {
			case WID_S_LEFT:
                ...
                char buffer[3];
                sprintf(buffer, "%02d", _hour);
                SetDParamStr(1, buffer);
				d = GetStringBoundingBox(STR_WHITE_DATETIME);
				break;
	...
	virtual void DrawWidget(const Rect &r, int widget) const
	{
		switch (widget) {
			case WID_S_LEFT:
				...
				DrawString(r.left + WD_FRAMERECT_LEFT, r.right - WD_FRAMERECT_RIGHT, r.top + WD_FRAMERECT_TOP, STR_WHITE_DATETIME, TC_FROMSTRING, SA_HOR_CENTER);
				break;
