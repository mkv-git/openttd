...
static inline Money RoadMaintenanceCost(RoadType roadtype, uint32 num)
{
	...
	return ((_price[PR_INFRASTRUCTURE_ROAD] * (roadtype == ROADTYPE_TRAM ? 3 : 2) * num * (1 + IntSqrt(num))) >> 9) * COST_MULTIPLIER; // 2 bits fraction for the multiplier and 7 bits scaling.
}
